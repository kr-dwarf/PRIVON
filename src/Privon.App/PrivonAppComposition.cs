...
/// session-lock sensitive-state-discard capability. Owns the ONE process-wide instance of every
/// shared-identity leaf component (<see cref="ClipboardOperationGate"/>/
/// <see cref="ClipboardDecisionScopeLifecycle"/>/<see cref="ClipboardComposerVerifier"/>/
/// <see cref="ClipboardChangeMonitor"/>/<see cref="ComposerTextReader"/>/
/// <see cref="ISessionLockNotification"/>), wires the real production object graph exactly once, and
/// starts/stops it in the frozen order (Phase 3C STEP35/STEP35.1, extended STEP38). This is a plain
/// CLR class -- no DI container/framework, no WPF <c>Application</c>/<c>Window</c>/<c>Dispatcher</c>
/// dependency of any kind -- so it can be constructed and exercised deterministically from
/// <c>Privon.App.Tests</c> without ever instantiating <see cref="System.Windows.Application"/>.
/// <c>App.xaml.cs</c> is a thin adapter that owns exactly one instance of this type.
///
...
///
/// OBJECT_GRAPH / SHARED_IDENTITY (frozen): exactly one <see cref="ClipboardOperationGate"/>, one
/// <see cref="ClipboardDecisionScopeLifecycle"/> (backing all three of
...
/// transports), one <see cref="ComposerTextReader"/>, and one <see cref="ISessionLockNotification"/>
...
/// references it.
///
/// DIRECT_OWNERSHIP (frozen minimal set, extended STEP38): only the components that need
/// lifecycle/disposal/entrypoint ownership are held as fields -- <see cref="ISessionLockNotification"/>/
/// <see cref="ClipboardChangeMonitor"/>/<see cref="ComposerTextReader"/>/
...
/// live consumers (the read/write/composer-read transports, the trust/exception provider, the privacy
/// processor, the decision-session publisher, the foreground target capture) are deliberately NOT
/// duplicated as separate fields here -- GC reachability through the coordinator/resolver/verifier is
/// sufficient, and a duplicate field would only be dead weight.
///
/// START_ORDER (frozen, Phase 3C STEP35.1, revised STEP38): the ENTIRE object graph is fully
/// constructed (including subscribing <see cref="OnSessionLocked"/> to
...
/// -- that coordinator call is what ultimately starts <see cref="ClipboardChangeMonitor"/> (via its
/// own internal <see cref="IClipboardReadTransport.Start"/> call); this type never calls
/// <see cref="ClipboardChangeMonitor.Start"/> directly, and never calls
/// <see cref="Privon.Windows.SessionLockMonitor.Start"/> directly either (only ever through
/// <see cref="ISessionLockNotification"/>). The session-lock observer becomes live BEFORE clipboard
/// protection does, so a lock event can never be missed once protection begins (Phase 3C STEP38
/// instruction's explicit ordering preference) -- but its own callback targets
...
///
/// STARTUP_FAILURE_ROLLBACK (frozen, revised STEP38): a single <c>try</c>/<c>catch</c> wraps graph
/// construction, the session-lock-observer start, the composer-reader start, and the coordinator
/// start together -- ANY exception at ANY of those four stages triggers the identical
...
/// type.
///
/// SHUTDOWN_ORDER (frozen, Phase 3C STEP36 instruction, revised STEP38): the session-lock observer is
/// unsubscribed and stopped FIRST -- <see cref="ISessionLockNotification.Locked"/> -=
...
/// <see cref="ClipboardPrivacyCoordinator.Dispose"/> -&gt;
...
/// -&gt; <see cref="ClipboardChangeMonitor.Dispose"/> -&gt; <see cref="ClipboardOperationGate.Dispose"/>.
/// <see cref="Dispose"/> runs every step even if an earlier one fails (each wrapped independently), so
/// the sensitive-state-discard steps always run regardless of an observer/coordinator/reader/monitor
/// cleanup failure -- but this instance is only marked fully disposed once every step succeeded
/// (mirroring this codebase's established DISPOSE_FAILURE_BEHAVIOR discipline: a later retry
/// genuinely retries cleanup rather than silently no-op'ing over an unresolved failure); the FIRST
/// failure encountered is what ultimately propagates. <see cref="ISessionLockNotification"/> declares
/// no <c>Dispose</c> member of its own -- <see cref="ISessionLockNotification.Stop"/> alone already
/// achieves full, deterministic teardown (unregister/destroy/join), exactly like calling
/// <see cref="Privon.Windows.SessionLockMonitor.Stop"/> directly would.
///
...
internal sealed class PrivonAppComposition : IDisposable
{
    ...
    // _sessionLockNotification/_monitor/_composerReader are assigned in the constructor (construction
    // alone starts no thread and performs no I/O); every other direct-ownership field is assigned only
    // once graph construction inside Start() reaches it, so a startup failure never leaves a field
    // pointing at something this type never actually built.
    ...
    /// <see cref="ClipboardChangeMonitor"/>/<see cref="ComposerTextReader"/> against the frozen
    /// production storage root, and enables the metadata-only diagnostic recorder ONLY when
    /// <see cref="DiagnosticsEnabledFromEnvironment"/> reports the explicit opt-in environment
    /// variable is set. A normal public launch (that variable unset, which is every real end
    /// user's machine) therefore never constructs <see cref="FileClipboardDiagnosticRecorder"/> at
    /// all -- no <c>%TEMP%\privon-diagnostic-*.log</c> file is ever created, no background writer
    /// thread is ever started, by default.
    /// </summary>
    public PrivonAppComposition()
        : this(ProductionStorageRootPath, new SessionLockNotificationAdapter(), new ClipboardChangeMonitor(), new ComposerTextReader(), DiagnosticsEnabledFromEnvironment())
    {
    }
    ...
    /// instance (to deterministically reproduce the real, documented "second Start() always throws"
    /// failure this type's own rollback logic must handle) -- without a mocking framework and without
    /// bypassing the real production object graph for anything else. The production (public)
    /// constructor above always supplies fresh, unstarted real instances.
    ///
