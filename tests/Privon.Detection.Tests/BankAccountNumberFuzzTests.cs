...
// Property/fuzz-style tests for BankAccountNumberDetector, mirroring FuzzTests.cs's approach:
// no external library added, System.Random with a fixed seed instead. All generated strings
// are structurally-random noise -- never real PII.
public class BankAccountNumberFuzzTests
{
    ...
    public void RandomStrings_NeverThrow_AndSpansAreAlwaysValid()
    {
        ...
        for (int iteration = 0; iteration < 2000; iteration++)
        {
            ...
            var results = new BankAccountNumberDetector().Detect(context);
    ...
    public void RandomDigitRuns_NeverThrow()
    {
        ...
        for (int iteration = 0; iteration < 500; iteration++)
        {
            ...
            var view = NormalizedView.Build(text);
            var results = new BankAccountNumberDetector().Detect(new DetectionContext(view));
